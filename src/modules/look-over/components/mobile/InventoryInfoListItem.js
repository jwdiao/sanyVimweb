...
import {inventoryInfoListItemsMap} from "../../../../utils/ReportsRelatedConstants";

const _ = require('lodash')

class _InventoryInfoListItem extends Component {
    render() {
                ...
                <ContentView>
                    {
                        keys.map((_key, index)=>{
                            let val = data[_key];
                            if (_.indexOf(['quantity', 'inInventoryQuantity', 'qualifiedQuantity', 'unqualifiedQuantity', 'onTheWayQuantity'], _key) >= 0) {
                                val = val + ' ' + units;
                            }
                            return (
                                <ItemWrapper
                                    key={_key}>
                                    <ItemView>
                                        <TitleText>{inventoryInfoListItemsMap[_key]}</TitleText>
...
export const InventoryInfoListItem = _InventoryInfoListItem;
